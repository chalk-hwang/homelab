...
ENDPOINT=${NORDVPN_ENDPOINT:-Japan}
...
nordvpn_connect() {
  ...
  while [ 1 ]; do
    ...
    if [ $? -eq 0 ]; then
      bash /scripts/nordvpn_iptables_rules.sh add
      return 0
    fi
    sleep 1
...
nordvpn allowlist add subnet 10.0.0.0/16
nordvpn allowlist add subnet 100.64.0.0/10
nordvpn allowlist add subnet 10.2.0.0/23
nordvpn allowlist add port 41650
...
while [ 1 ]; do
  ...
  nordvpn status
  nordvpn disconnect
  bash /scripts/nordvpn_iptables_rules.sh del
  sleep 1
  nordvpn_connect
  echo
done